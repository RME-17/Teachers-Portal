...
// ffmpeg-static to app.asar.unpacked, but the resolved path still points at
// app.asar, so rewrite it to the unpacked location for spawn().
if (typeof ffmpegPath === "string" && ffmpegPath.includes("app.asar") && !ffmpegPath.includes("app.asar.unpacked")) {
	ffmpegPath = ffmpegPath.replace("app.asar", "app.asar.unpacked");
}
// Prefer an explicitly-configured or PATH ffmpeg over the bundled ffmpeg-static
// binary. A missing/non-executable ffmpeg-static path caused spawn failures that
// emitted raw 24kHz PCM wrapped in a 48kHz WAV header (high-pitched screech / no audio).
;(function resolveStudioFfmpeg() {
	const fs = require("fs");
	const tryPath = (c) => { try { return c && fs.existsSync(c) ? c : null; } catch { return null; } };
	const envBin = tryPath(process.env.RME_FFMPEG_BIN);
	if (envBin) { ffmpegPath = envBin; return; }
	if (tryPath(ffmpegPath)) return;
	try {
...
function streamThroughMaster({ inputStream, inputSampleRate, inputBitDepth, signal }) {
	...
	child.on("error", (err) => {
		...
		// Do NOT pipe raw input PCM out here: it is at inputSampleRate but would be
		// wrapped with the 48kHz output header (high-pitched screech). Destroy the
		// stream so processWavBuffer falls back to the original (correct) WAV.
		try { inputStream.unpipe(child.stdin); } catch {}
		if (!aborted) outStream.destroy(err);
	});
