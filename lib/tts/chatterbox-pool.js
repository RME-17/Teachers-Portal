...
function createChatterboxPool(opts = {}) {
  ...
  const scriptPath = path.join(process.env.RME_TOOLS_ROOT || path.join(APP_ROOT, 'tools'), 'tts', 'chatterbox-server.py');
  ...
  async function spawnPort(port) {
    ...
    const proc = spawn(pythonCmd, [scriptPath, ...modelArgs, '--port', String(port)], {
      ...
      env: Object.assign({}, process.env, { PYTHONUNBUFFERED: '1', TQDM_DISABLE: '1' },
