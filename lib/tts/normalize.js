...
function normalizeForTts(text, opts) {
  ...
  /* Protect prosody tags from the character-level cleanup below */
  if (preserveProsody) {
    t = t.replace(/\[pause\s*=\s*(\d+)\]/gi, "<<PAUSE_$1>>");
    t = t.replace(/\[\/slow\]/gi, "<<SLOW_END>>");
    t = t.replace(/\[slow\]/gi, "<<SLOW>>");
    t = t.replace(/\[\/fast\]/gi, "<<FAST_END>>");
    t = t.replace(/\[fast\]/gi, "<<FAST>>");
    t = t.replace(/\[\/emph\]/gi, "<<EMPH_END>>");
    t = t.replace(/\[emph\]/gi, "<<EMPH>>");
  }
  t = t.replace(/\[chuckle\]/gi, "<<CHUCKLE>>");
  t = t.replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE00}-\u{FE0F}\u{200D}\u{20E3}]/gu, " ");
  t = t.replace(/[^\w\s.,!?;:'"()\-]/gu, " ");
  if (preserveProsody) {
    t = t.replace(/<<PAUSE_(\d+)>>/g, "[pause=$1]");
    t = t.replace(/<<SLOW_END>>/g, "[/slow]");
    t = t.replace(/<<SLOW>>/g, "[slow]");
    t = t.replace(/<<FAST_END>>/g, "[/fast]");
    t = t.replace(/<<FAST>>/g, "[fast]");
    t = t.replace(/<<EMPH_END>>/g, "[/emph]");
    t = t.replace(/<<EMPH>>/g, "[emph]");
  }
