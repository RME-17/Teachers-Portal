...
async function synthesizeStreaming(opts) {
	...
	try {
		...
		// Release serial slot early — stream is established, next sentence
		// can send its HTTP request to the Python server immediately.
		// The server will queue it behind this stream.
		releaseRequest();
		slotReleased = true;
		...
		if (port !== null && ttsPool) ttsPool.release(port);
		if (!slotReleased) releaseRequest();
	}
}
