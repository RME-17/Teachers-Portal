...
function getTtsVoice() {
	return _currentVoice || String(process.env.RME_CHATTERBOX_VOICE || "ryan-english").trim() || "ryan-english";
}
...
async function ensureServer() {
  ...
  try {
    ...
    if (POOL_SIZE === 1) {
      ...
      const scriptPath = path.join(process.env.RME_TOOLS_ROOT || path.join(__dirname, "..", "..", "tools"), "tts", "chatterbox-server.py");
      ...
      sp.on("error", (err) => {
        console.warn("[chatterbox] spawn error:", (err && err.message) || err);
        _ready = false;
      });
      sp.stdout.on("data", (d) => {
