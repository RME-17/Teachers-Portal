...
function getParakeetServerConfig() {
	...
	const model = String(process.env.RME_PARAKEET_MODEL || "nvidia/parakeet-tdt-0.6b-v2").trim();
...
async function ensureParakeetServer() {
	...
	startPromise = (async () => {
		...
		const env = {
			...
			RME_PARAKEET_BOOST_WORDS: process.env.RME_PARAKEET_BOOST_WORDS || "Talking Global,Magic English,Speak English,Nice Kid,Sky Line,RecruitMyEnglish,Tessa Jean Lund,Ilonka,Joline,Megan,Maureen Vermaak,Amanda Uys,Deirdre Botes,Bavna Garridigan,Chante,Medina,Lanatha Eayrs,Lana Eayrs,Theresa Van Niekerk,Janina Swart,Kayleigh Ogle",
		};

		serverProc = spawn(cfg.python, args, {
			// APP_ROOT lives inside app.asar (a file) when packaged; using it as cwd
			// makes spawn throw ENOENT (blaming the python path). Use a real dir.
			cwd: path.dirname(cfg.scriptPath),
			windowsHide: true,
		...
		serverProc.on("error", (err) => {
			console.warn(`[parakeet] spawn failed (using Whisper STT instead): ${(err && err.message) || err}`);
			serverProc = null;
			startPromise = null;
		});
