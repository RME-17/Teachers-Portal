...
function buildToolDefs(client, opts = {}) {
    ...
    defs.push({
        ...
        description: 'Broadcast a message to EVERY channel whose name matches a pattern (default ALL "clock-in" channels). Each clock-in channel sits under its own school category, so this posts to all schools at once and reports back per school using the parent category name. Use this whenever asked to post the same message across the clock-in channels (for example a daily good-morning or good-afternoon) instead of discord_send_message, which only hits ONE channel. INPUT: { content: string, namePattern?: string (default "clock-in"), category?: string (optional: only send to channels whose parent category name contains this, e.g. a single school) }',
        input_schema: { type: 'object', properties: { content: { type: 'string' }, namePattern: { type: 'string' }, category: { type: 'string' } }, required: ['content'] },
    });
...
module.exports = { buildToolDefs, callTool, autoResolveMessageRef, getLastSentMessage, getRecentSeenMessages, WRITE_ALLOWLIST, DRAFT_HOLDING_CHANNELS };
