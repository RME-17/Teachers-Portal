...
function buildToolDefs(client, opts = {}) {
    ...
    defs.push({
        ...
        description: 'Send a message to a channel. SAFETY: every send is drafted to #bot-drafts first and returns CONFIRM_REQUIRED with a one-time code; the human reads that code back, then you call again with it as confirmationToken to actually send. Draft-only if channel not allowlisted. INPUT: { channel: string, content: string, confirmationToken?: string }',
        input_schema: { type: 'object', properties: { channel: { type: 'string' }, content: { type: 'string' }, confirmationToken: { type: 'string', description: 'One-time code the user reads back from #bot-drafts. Required to actually send; never invent or guess it.' } }, required: ['channel','content'] },
    });

    defs.push({
        name: 'discord_send_dm',
        description: 'Send a DM to a user. SAFETY: the first call drafts the DM to #bot-drafts and returns CONFIRM_REQUIRED with a one-time code; the human reads that code back, then call again with it as confirmationToken to actually deliver the DM. INPUT: { userId: string (discord id) OR username: string (discord username or nickname), content: string, confirmationToken?: string }',
        input_schema: {
            type: 'object',
            properties: {
                ...
                content: { type: 'string' },
                confirmationToken: { type: 'string', description: 'One-time code the user reads back from #bot-drafts. Required to actually deliver the DM; never invent it.' }
            },
            required: ['content']
        },
    });
    ...
    defs.push({
        ...
        description: 'Broadcast a message to EVERY channel whose name matches a pattern (default ALL "clock-in" channels). Each clock-in channel sits under its own school category, so this posts to all schools at once and reports back per school using the parent category name. Use this whenever asked to post the same message across the clock-in channels (for example a daily good-morning or good-afternoon) instead of discord_send_message, which only hits ONE channel. SAFETY: first call drafts to #bot-drafts and returns CONFIRM_REQUIRED; the human reads the one-time code back, then call again with it as confirmationToken to broadcast. INPUT: { content: string, namePattern?: string (default "clock-in"), category?: string (optional: only send to channels whose parent category name contains this, e.g. a single school) }',
        input_schema: { type: 'object', properties: { content: { type: 'string' }, namePattern: { type: 'string' }, category: { type: 'string' }, confirmationToken: { type: 'string', description: 'One-time code the user reads back from #bot-drafts. Required to actually broadcast; never invent it.' } }, required: ['content'] },
    });

    defs.push({
        name: 'discord_edit_last_broadcast',
        description: 'Edit the most recent discord_send_all broadcast in place across every channel it was posted to. SAFETY: first call drafts the new text to #bot-drafts and returns CONFIRM_REQUIRED with a one-time code; the human reads it back, then call again with it as confirmationToken to apply the edit. INPUT: { content: string (the new message text), confirmationToken?: string }',
        input_schema: { type: 'object', properties: { content: { type: 'string' }, confirmationToken: { type: 'string', description: 'One-time code the user reads back from #bot-drafts. Required to actually edit; never invent it.' } }, required: ['content'] },
    });

    defs.push({
        name: 'discord_retract_last_broadcast',
        description: 'Delete the most recent discord_send_all broadcast from every channel it was posted to. SAFETY: first call drafts a retraction notice to #bot-drafts and returns CONFIRM_REQUIRED with a one-time code; the human reads it back, then call again with it as confirmationToken to delete. INPUT: { confirmationToken?: string }',
        input_schema: { type: 'object', properties: { confirmationToken: { type: 'string', description: 'One-time code the user reads back from #bot-drafts. Required to actually retract; never invent it.' } }, required: [] },
    });
                ...
                try {
                    ...
                    // and is NEVER returned to the model, so the AI cannot self-confirm.
                    {
                        ...
                        if (!_conf.ok) {
                            ...
                            return { ok: true, data: [{ type: 'text', text: `CONFIRM_REQUIRED: drafted to #${DRAFT_HOLDING_CHANNELS.modDrafts} for #${channel.name}${_why}. A one-time confirmation code was posted there. Ask the user to read the code back, then call discord_send_message again with that exact confirmationToken. NEVER invent or guess the code.` }], isError: false, ms: 0 };
                        }
                ...
                try {
                    ...
                    // and never returned to the model, so the AI cannot self-confirm.
                    {
                        ...
                        if (!_conf.ok) {
                            ...
                            return { ok: true, data: [{ type: 'text', text: `CONFIRM_REQUIRED: drafted to #${DRAFT_HOLDING_CHANNELS.modDrafts} for ${matching.length} channel(s):\n` + preview.join('\n') + _warn + `\n\nA one-time confirmation code was posted to #${DRAFT_HOLDING_CHANNELS.modDrafts}. Ask the user to read it back, then call discord_send_all again with that exact confirmationToken to broadcast. NEVER invent the code.` }], isError: false, ms: 0 };
                        }
                        _consumeConfirmation(_conf.key);
                    }
                ...
                try {
                    ...
                    // and never returned to the model; the human reads it back to deliver.
                    {
                        ...
                        if (!_conf.ok) {
                            ...
                            return { ok: true, data: [{ type: 'text', text: `CONFIRM_REQUIRED: drafted this DM to #${DRAFT_HOLDING_CHANNELS.modDrafts}. A one-time confirmation code was posted there. Ask the user to read it back, then call discord_send_dm again with that exact confirmationToken. NEVER invent the code.` }], isError: false, ms: 0 };
                        }
                        _consumeConfirmation(_conf.key);
                    }
                ...
                try {
                    ...
                    const chunks = chunkForDiscord(safeContent);
                    {
                        ...
                        if (!_conf.ok) {
                            ...
                            return { ok: true, data: [{ type: 'text', text: `CONFIRM_REQUIRED: drafted the edit to #${DRAFT_HOLDING_CHANNELS.modDrafts}. A one-time confirmation code was posted there. Ask the user to read it back, then call discord_edit_last_broadcast again with that exact confirmationToken. NEVER invent the code.` }], isError: false, ms: 0 };
                        }
                        _consumeConfirmation(_conf.key);
                    }
                    const summary = [];
                ...
                try {
                    ...
                    const _stamp = 'retract:' + _lastBroadcast.at;
                    {
                        ...
                        if (!_conf.ok) {
                            ...
                            return { ok: true, data: [{ type: 'text', text: `CONFIRM_REQUIRED: posted a retraction confirmation code to #${DRAFT_HOLDING_CHANNELS.modDrafts}. Ask the user to read it back, then call discord_retract_last_broadcast again with that exact confirmationToken. NEVER invent the code.` }], isError: false, ms: 0 };
                        }
                        _consumeConfirmation(_conf.key);
                    }
                    const summary = [];
