...
function buildToolDefs(client, opts = {}) {
    ...
    // status/health tool - listed first so the model always sees it
    defs.push({
        name: 'discord_status',
        description: 'Check whether the bot is ACTUALLY connected to Discord right now. Returns a live verdict (CONNECTED, LOGGING_IN, NOT_CONNECTED, LOGGED_IN_BUT_GUILD_UNREACHABLE) based on real client state: bot identity, gateway ready, ping, configured guild reachability, channel counts, and the last message actually sent. ALWAYS call this when asked whether you are connected/online/live. NEVER claim you are connected or that a server is reachable without calling this first and reporting its verdict. INPUT: none',
...
async function callTool(client, name, input = {}, triggerContext = 'unknown', turnId) {
    // discord_status reports the TRUE connection state and MUST run before the not-ready gate below.
    if (name === 'discord_status') {
        return buildDiscordStatus(client);
    }
...
function buildDiscordStatus(client) {
    try {
        const loggedIn = !!(client && client.user);
        const gatewayReady = !!(client && typeof client.isReady === 'function' ? client.isReady() : loggedIn);
        const ping = (client && client.ws && typeof client.ws.ping === 'number' && isFinite(client.ws.ping)) ? Math.round(client.ws.ping) : null;
        const botTag = loggedIn ? (client.user.tag || client.user.username || null) : null;
        const rawGuild = String(process.env.DISCORD_GUILD_ID || '').trim();
        const gm = rawGuild.match(/(\d{16,20})/);
        const wantGuildId = gm ? gm[1] : null;
        let guild = null;
        try {
            if (client && client.guilds && client.guilds.cache) {
                guild = wantGuildId ? client.guilds.cache.get(wantGuildId) : (client.guilds.cache.first() || null);
            }
        } catch (e) {}
        let channelInfo = '';
        if (guild) {
            try {
                const text = Array.from(guild.channels.cache.values()).filter(c => c && (c.type === 0 || c.type === 'GUILD_TEXT' || (c.isTextBased && c.isTextBased())));
                const clockIn = text.filter(c => /clock-in/i.test(c.name || ''));
                const hasModDrafts = !!text.find(c => /mod-drafts/i.test(c.name || ''));
                channelInfo = ' Server "' + guild.name + '" reachable - ' + text.length + ' text channels (' + clockIn.length + ' clock-in; mod-drafts ' + (hasModDrafts ? 'present' : 'MISSING') + ').';
            } catch (e) { channelInfo = ' (channel enumeration failed: ' + (e && e.message) + ')'; }
        }
        let last = null;
        try { last = getLastSentMessage(); } catch (e) {}
        const lastTxt = last ? (' Last message actually sent this session: "' + last.snippet + '" in #' + last.channelName + '.') : ' No message has actually been sent this session.';
        let verdict;
        if (loggedIn && gatewayReady && guild) verdict = 'CONNECTED';
        else if (loggedIn && gatewayReady && !guild) verdict = wantGuildId ? 'LOGGED_IN_BUT_GUILD_UNREACHABLE' : 'LOGGED_IN_NO_GUILD_CONFIGURED';
        else if (loggedIn && !gatewayReady) verdict = 'LOGGING_IN';
        else verdict = 'NOT_CONNECTED';
        const text = 'DISCORD_STATUS=' + verdict + '. loggedIn=' + loggedIn + (botTag ? ' (' + botTag + ')' : '') + ', gatewayReady=' + gatewayReady + (ping != null ? ', pingMs=' + ping : '') + ', guildConfigured=' + (wantGuildId || 'NONE') + ', guildReachable=' + (!!guild) + '.' + channelInfo + lastTxt + ' This is the real live state. Report the verdict honestly; only say you are connected if verdict is CONNECTED.';
        return { ok: true, data: [{ type: 'text', text: text }], isError: false, ms: 0 };
    } catch (err) {
        return { ok: true, data: [{ type: 'text', text: 'DISCORD_STATUS=UNKNOWN (status check threw: ' + (err && err.message || String(err)) + '). Treat as NOT connected until verified.' }], isError: false, ms: 0 };
    }
}

module.exports = { buildToolDefs, callTool, buildDiscordStatus, _sendVerified, autoResolveMessageRef, getLastSentMessage, getRecentSeenMessages, WRITE_ALLOWLIST, DRAFT_HOLDING_CHANNELS, getLastBroadcast, _issueConfirmation, _checkConfirmation, _consumeConfirmation, _broadcastDedupe };
