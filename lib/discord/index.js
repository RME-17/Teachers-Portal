    ...
    const onReady = async () => {
        ...
        try {
            ...
            for (const want of allowNames) {
                ...
                let chosen = /^\d{17,20}$/.test(want) ? (client.channels.cache.get(want) || null) : null;
                if (!chosen) {
                    const list = nameToChannels.get(want) || [];
                    // prefer a text-based channel when available
                    chosen = list.find(c => c.isTextBased && c.isTextBased()) || list[0] || null;
                }
                if (chosen) {
                    aiTools.WRITE_ALLOWLIST.add(String(chosen.id));
                    resolved.push({ name: chosen.name || want, id: chosen.id });
                } else {
                    missing.push(want);
                }
            }
