...
async function storeTurn({ userEmail, role, content, cid, userName }) {
	try {
		...
		if (!emb.ok) {
			...
			console.log("[memory] stored turn:", role, "id=" + rowId, "cid=" + cid, "chars=" + (content ? content.length : 0));
...
async function getMemoryDigest({ userEmail, maxTokens = 500, userName, speakerId }) {
	try {
		const sId = speakerId || userName || null;
		// Load top active facts sorted by recency
		const factsResult = await listFacts({ userEmail, userName, speakerId: sId });
		// Load most recent summaries
		const summariesResult = await getRecentSummaries({ userEmail, k: 2 });
		// Load last N raw turns for conversational continuity
		const recentTurnsResult = await getRecentConversations({
			userEmail, userName, speakerId: sId, limit: 10,
		});
