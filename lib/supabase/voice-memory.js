...
async function storeTurn({ userEmail, role, content, cid, userName }) {
	// v1.0.31: never persist canned/transient error replies into memory (they poison future context)
	if (role === "assistant" && typeof content === "string" && /anthropic api key is missing|could not reach notion|hit an error reaching my language model|having trouble pinning that down/i.test(content)) {
		console.log("[memory] skip storing canned error turn (not persisted)");
		return null;
	}
	try {
